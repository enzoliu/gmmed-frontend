...
本系統支援**永久保固功能**，適用於特殊產品：

- **永久保固產品**：`warranty_years = -1` 表示永久保固
- **無保固產品**：`warranty_years = 0` 表示無保固
- **保固期限**：永久保固產品的保固結束日期設為 `9999-12-31`
- ✅ **統計排除**：永久保固不計入即將到期統計
- **清楚標示**：前端可以根據 `warranty_years` 值判斷並顯示「永久保固」
...
1. **保固期限顯示**：
   ...
   - 檢查 `product.warranty_years` 是否為 -1
   - 如果為 -1，顯示「永久保固」而不是具體的到期日期
   - 如果為 0，顯示「無保固」
   - 例如：`warranty_years === -1 ? "永久保固" : warranty_years === 0 ? "無保固" : "保固至 " + warranty_end_date`
...
- ✅ **永久保固**：支援 `warranty_years = -1` 的永久保固產品
- ✅ **無保固**：支援 `warranty_years = 0` 的無保固產品
...
function isWarrantyExpired(warranty) {
  // 永久保固永不過期
  if (warranty.product.warranty_years === -1) {
    return false;
  }
