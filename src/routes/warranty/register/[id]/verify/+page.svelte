<script lang="ts">
  ...
  let shouldDisplayAgreement = false;
  ...
  function loadAgreement() {
    ...
    if (!agreement) {
      shouldDisplayAgreement = true;
    }
  }

  function handleAgree() {
    localStorage.setItem("agreement", "true");
    shouldDisplayAgreement = false;
  }
...
{#if shouldDisplayAgreement}
  <Stepper step={1} />
  <Agreements onAgree={handleAgree} displayOnly={false} />
{:else}
  <Stepper step={2} />
  <div class="container mx-auto max-w-3xl py-8 px-4">
    ...
    >
      <div class="p-4">
        ...
        <p class="text-center text-gray-600 mb-2">請輸入序號進行驗證</p>
        ...
        <form class="space-y-6">
          ...
          >
            ...
            <SerialInput
              ...
              {warrantyId}
              onUpdate={(s1, s2) => {
                formData.product_serial_number = s1.serial_number;
                formData.product_serial_number_2 = s2.serial_number;
                formData.checksum = s1.checksum;
                formData.checksum2 = s2.checksum;
              }}
            />
          </div>
